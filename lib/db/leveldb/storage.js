    ...
    function LevelDownStorage(uri) {
        ...
        var getBlocksByHashes = this.getBlocksByHashes =
            function getBlocksByHashes(hashes, callback) {
                Step(
                    function () {
                        console.log('getBlocksByHashes: 1')
                        var group = this.group();
                        for (var i = 0, l = hashes.length; i < l; i++) {
                            if (hashes[i]) {
                                hMain.get(hashes[i], defaultGetOpts, group());
        ...
        var getBlocksByHeights = this.getBlocksByHeights =
            function getBlocksByHeights(heights, callback)
            {
                Step(
                    function () {
                        var group = this.group();
                        console.log('heights args: ', heights)
                        heights.forEach(function (i) {
                            hMain.get(formatHeightKey(i), defaultGetOpts, group());
                            hMain.get(formatHeightKey(i), defaultGetOpts, function (err, val) {
                                console.log('i: ' + i + ', err: ' + err + ', val: ' + val);
                            });
                        });
                        console.log('level db: getBlocksByHeights: 1')
                    },
                    function (err, hashes) {
                        console.log('err: ', err)
                        console.log('hashes: ', hashes)
                        if (err) throw err;
                        console.log('level db: getBlocksByHeights: 2')

                        getBlocksByHashes(hashes, this);
                    },
                    function sortStep(err, blocks) {
                        if (err) throw err;
                        console.log('level db: getBlocksByHeights: 3')
        ...
        var getTopBlock = this.getTopBlock =
            function getTopBlock(callback) {
                console.log('level db: getTopBlock')
                console.log('metadata.chainHeight: ', metadata.chainHeight)
