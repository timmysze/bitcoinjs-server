...
var BlockChain = exports.BlockChain = function BlockChain(storage, settings) {
  ...
  var add = this.add = function add(block, txs, callback) {
    console.log('add to block chain')
    var self = this;
  ...
  var processBlock = this.processBlock = function processBlock(bw) {
    ...
    Step(
      function prepareStep() {
        console.log('process block: prepare step')
        isProcessing = true;
      ...
      function connectStep(err) {
        console.log('process block: connect step')
        if (err) throw err;
      ...
      function verifyConnectionStep(err) {
        if (err) throw err;
        console.log('process block: verify connection step')

        if (!self.cfg.verify) {
          this();
          return;
        }
        console.dir(bw.parent.verifyChild)
        bw.parent.verifyChild(self, bw.block, this);
      },
      function applyParentStep(err) {
        console.log('process block: apply parent step')
        if (err) throw err;
      ...
      function prepareTxsStep(err) {
        console.log('process block: prepare txs step')
        if (err) throw err;
      ...
      function verifyBlockStep(err) {
        ...
        verifyBlock(bw, this);
        console.log('process block: verify block step')
      },
      function startTransactionStep(err) {
        console.log('process block: start transaction step')
        if (err) throw err;

        storage.startTransaction(this);
      },
      function reorganizeStep(err) {
        console.log('process block: reorganize step')
        if (err) throw err;
      ...
      function saveTransactionsStep(err) {
        console.log('process block: save transactions step')
        if (err) throw err;

        self.saveTransactions(bw.block, bw.txs, this);
      },
      function saveBlockStep(err) {
        console.log('process block: save block step')
        if (err) throw err;

        self.saveBlock(bw, this);
      },
      function connectTransactionsStep(err) {
        console.log('process block: connect transactions step')
        if (err) throw err;
      ...
      function endTransactionStep(err) {
        console.log('process block: end transaction step')
        if (err) throw err;

        storage.endTransaction(this);
      },
      function queueDependentsStep(err) {
        console.log('process block: queue dependents step')
        if (err) throw err;
      ...
      function finalizeStep(err) {
        console.log('process block: finalize step')
